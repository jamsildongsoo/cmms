...
export default function WorkPermitRegisterPage() {
    ...
    const onSubmit = async (data: WorkPermit) => {
        try {
            if (isEditMode && id) {
                await workPermitService.update(id, data);
                toast({ title: "성공", description: "작업허가 신청이 수정되었습니다." });
            } else {
                await workPermitService.create(data);
                toast({ title: "성공", description: "작업허가 신청이 등록되었습니다." });
            }
        ...
        <div className="max-w-5xl mx-auto space-y-6 pb-10">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button variant="outline" type="button" onClick={() => navigate('/wp/work-permit')}>취소</Button>
                    <Button type="submit" form="wp-form" className="bg-red-600 hover:bg-red-700 text-white">
                        <Save className="mr-2 h-4 w-4" /> 신청
                    </Button>
                </div>
            </div>
            ...
            <form id="wp-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <Card>
                    ...
                    <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        {/* Row 1: Permit ID (1) / Name (2) / Date (1) */}
                        ...
                        <div className="space-y-2 md:col-span-2">
                            ...
                            <Input {...register('name', { required: true })} placeholder="예: 배관 용접 작업" />
                        </div>
                        <div className="space-y-2">
                            <Label>신청일자</Label>
                            {/* Using start_dt as representative date for now, or add a proper request date field */}
                            <Input type="date" disabled value={new Date().toISOString().split('T')[0]} className="bg-slate-50" />
                        </div>

                        {/* Row 2: Equipment (1) / Dept (1) / Person (1)  - Removed Type from here */}
                        <div className="space-y-2">
                            <Label>대상 설비</Label>
                            <SearchableSelect
                        ...
                        <div className="space-y-2">
                            ...
                            <SearchableSelect
                                ...
                                onChange={(id) => setValue('person_name', id)}
                                placeholder="신청자 검색..."
                                displayFormat={(person) => `${person.name} ${person.position || ''} (${person.person_id})`}
                            />
                        </div>
                        <div className="space-y-2">
                            {/* Empty spacer or move fields to fill */}
                        </div>

                        {/* Special Work Types Selection (New Row) */}
                        ...
                        {/* Row 3: Start (1) / End (1) / Location (1) */}
                        <div className="space-y-2">
                            <Label>시작 일시</Label>
                            <Input type="datetime-local" {...register('start_dt')} />
                        </div>
                        <div className="space-y-2">
                            <Label>종료 일시</Label>
                            <Input type="datetime-local" {...register('end_dt')} />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                            <Label>작업 장소</Label>
                            <Input {...register('location')} placeholder="작업 위치 상세" />
                        </div>

                        {/* Row 4: Description (Full) */}
                        <div className="space-y-2 md:col-span-4">
                            <Label>작업 내용</Label>
                            <Textarea {...register('description')} placeholder="작업 내용을 상세히 기술하세요." className="min-h-[80px]" />
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="space-y-2">
                            <Label>작업 개요</Label>
                            <Input {...register('work_summary')} placeholder="주요 작업 내용 요약" />
                        </div>
                        <div className="space-y-2">
                            <Label>위험 요인</Label>
                            <Input {...register('hazard_factor')} placeholder="예: 화재, 추락, 질식" />
                        </div>
                        <div className="space-y-2 md:col-span-2">
                            <Label>안전 대책</Label>
                            <Input {...register('safety_factor')} placeholder="예: 소화기 비치, 안전벨트 착용" />
                        </div>
                    </CardContent>
                </Card>

                {/* 3. 공통 안전 점검 (Mandatory Common Checklist) */}
                <Card>
                    <CardHeader>
                    ...
                    <CardContent className="space-y-4">
                        <div className="flex items-center space-x-2">
                            <Checkbox id="com-check1" {...register('checksheet_json_com.check_ppe')} />
                            <Label htmlFor="com-check1">1. 개인보호구(안전모, 안전화 등)를 착용하였는가?</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                            <Checkbox id="com-check2" {...register('checksheet_json_com.check_edu')} />
                            <Label htmlFor="com-check2">2. 작업 전 안전 교육(TBM)을 실시하였는가?</Label>
                        </div>
                    </CardContent>
                </Card>

                {/* 4. Special Work Cards (Dynamic) */}
                {selectedTypes.includes('HOT') && (
                    <Card className="border-red-200 bg-red-50/10">
                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                            <div className="space-y-2">
                                <Label>화재 감시자</Label>
                                <Input {...register('checksheet_json_hot.fire_watcher')} placeholder="성명 입력" />
                            </div>
                            <div className="flex items-center space-x-2 pt-8">
                                <Checkbox id="hot-check1" {...register('checksheet_json_hot.fire_extinguisher')} />
                                <Label htmlFor="hot-check1">소화기 비치 및 이상 유무 확인</Label>
                            </div>
                            <div className="flex items-center space-x-2">
                                <Checkbox id="hot-check2" {...register('checksheet_json_hot.welding_blanket')} />
                                <Label htmlFor="hot-check2">불티 비산 방지포 설치 확인</Label>
                            </div>
                            <div className="flex items-center space-x-2">
                                <Checkbox id="hot-check3" {...register('checksheet_json_hot.gas_check')} />
                ...
                {selectedTypes.includes('CONF') && (
                    <Card className="border-blue-200 bg-blue-50/10">
                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4">
                            <div className="space-y-2">
                                <Label>산소 (O2) %</Label>
                                <Input {...register('checksheet_json_conf.gas_o2')} placeholder="18-23.5%" />
                            </div>
                            <div className="space-y-2">
                                <Label>일산화탄소 (CO) ppm</Label>
                                <Input {...register('checksheet_json_conf.gas_co')} placeholder="< 30ppm" />
                            </div>
                            <div className="space-y-2">
                                <Label>황화수소 (H2S) ppm</Label>
                                <Input {...register('checksheet_json_conf.gas_h2s')} placeholder="< 10ppm" />
                            </div>
                            <div className="space-y-2">
                                <Label>가연성가스 (LEL) %</Label>
                                <Input {...register('checksheet_json_conf.gas_lel')} placeholder="< 25%" />
                            </div>
                            <div className="md:col-span-4 flex items-center space-x-2">
                                <Checkbox id="conf-check1" {...register('checksheet_json_conf.ventilation')} />
