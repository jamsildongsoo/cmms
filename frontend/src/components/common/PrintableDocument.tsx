...
export const PrintableDocument: React.FC<PrintableDocumentProps> = ({ 
    documentType, stage, id, name, equipment, dept, person, date, items, note, companyName 
}) => {
    // Dynamic labels based on document type
    const isWO = documentType === 'WO';
    const typeLabel = isWO ? '작업지시' : '예방점검';
    const idLabel = isWO ? '지시번호' : '점검번호';
    const nameLabel = isWO ? '작업명' : '점검명';
    const dateLabel = isWO ? '작업일자' : '점검일자';
    const itemLabel = isWO ? '작업 항목' : '점검 항목';
    const methodLabel = isWO ? '작업 방법' : '점검 방법/기준';
    const resultLabel = isWO ? '조치 결과' : '점검 결과';

    const title = `${typeLabel} ${stage === 'PLN' ? '계획' : '실적'} 보고서`;
        ...
        <div className="print-container p-0 text-black bg-white w-full hidden print:block overflow-visible h-auto">
            ...
            <div className="p-[15mm] max-w-[800px] mx-auto">
                ...
                <div className="mb-6 space-y-2 text-sm">
                    <div className="flex">
                        <span className="font-bold w-24">{idLabel}:</span>
                        <span>{id}</span>
                    </div>
                    <div className="flex">
                        <span className="font-bold w-24">{nameLabel}:</span>
                        <span>{name}</span>
                    </div>
                    <div className="flex">
                        <span className="font-bold w-24">대상설비:</span>
                        <span>{equipment}</span>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-6 border-t border-slate-200 pt-2 mt-2">
                        <div className="flex">
                            <span className="font-bold mr-2">관리부서:</span>
                            <span>{dept}</span>
                        </div>
                        <div className="flex">
                            <span className="font-bold mr-2">담당자:</span>
                            <span>{person}</span>
                        </div>
                        <div className="flex">
                            <span className="font-bold mr-2">{dateLabel}:</span>
                            <span>{date}</span>
                        </div>
                        <div className="flex">
                            <span className="font-bold mr-2">상태:</span>
                            <span className="font-bold text-blue-700">완료</span>
                        </div>
                    </div>
                </div>
                ...
                {/* Items Table */}
                <div className="mb-10">
                    <div className="font-bold mb-1 text-sm">상세 내역:</div>
                    <table className="w-full border-collapse border border-black text-xs">
                        <thead className="bg-slate-50">
                            <tr>
                                <th className="border border-black p-2 w-10 text-center">No.</th>
                                <th className="border border-black p-2 text-center">{itemLabel}</th>
                                <th className="border border-black p-2 text-center">{methodLabel}</th>
                                <th className="border border-black p-2 w-24 text-center">{resultLabel}</th>
                        ...
                        <tbody>
                            {items.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="border border-black p-8 text-center text-slate-400">
                                        상세 내역이 없습니다.
                                    </td>
                                </tr>
                            )}
